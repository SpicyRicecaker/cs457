...
BmpToTexture(const char* filename, int* width, int* height)
{
  ...
  if (VERBOSE)
    fprintf(stderr,
            "FileHeader.bfType = 0x%0x = \"%c%c\"\n",
            FileHeader.bfType,
            FileHeader.bfType & 0xff,
            (FileHeader.bfType >> 8) & 0xff);
  ...
  if (VERBOSE)
    fprintf(
      stderr, "InfoHeader.biCompression = %d\n", InfoHeader.biCompression);
  ...
  int oldNumExtra =
    4 * (((3 * InfoHeader.biWidth) + 3) / 4) - 3 * InfoHeader.biWidth;
  if (VERBOSE)
  ...
  if (InfoHeader.biBitCount == 24) {
    ...
    if (InfoHeader.biCompression != 0) {
      fprintf(stderr,
              "Wrong type of image compression: %d\n",
              InfoHeader.biCompression);
      fclose(fp);
      return NULL;
    }
  ...
  if (InfoHeader.biBitCount == 8 && InfoHeader.biClrUsed == 256) {
    ...
    for (int c = 0; c < InfoHeader.biClrUsed; c++) {
      ...
      if (VERBOSE)
        fprintf(stderr,
                "%4d:\t0x%02x\t0x%02x\t0x%02x\t0x%02x\n",
                c,
                colorTable[c].r,
                colorTable[c].g,
                colorTable[c].b,
  ...
  if (InfoHeader.biBitCount == 32) {
    // 32-bit doesn't mind if the compression bits are set -- we just ignore
    // them:
    rewind(fp);
    fseek(fp, FileHeader.bfOffBytes, SEEK_SET);
    // numextra = 4 * (((4 * InfoHeader.biWidth) + 3) / 4) - 4 *
    // InfoHeader.biWidth;
    unsigned char* tp = texture;
    for (int t = 0; t < numt; t++) {
      for (int s = 0; s < nums; s++, tp += 3) {
