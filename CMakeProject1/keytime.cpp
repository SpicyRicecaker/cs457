...
double y_value_at_time(double t, double bias, double range, double x1, double y1, double x2, double y2) {
	return range * cubic_y(t, x1, y1, x2, y2) + bias;
}

float
Keytimes::GetValue(float time)
{
	...
	if (time <= tvs.front()->time)
		return tvs.front()->value;

	if (time >= tvs.back()->time)
	...
	int i0 = 0, i1 = 0;
	// t0 is beg time, v0 is val at begin
	// t1 is end time, v1 is val at end
	float t0 = 0., t1 = 0.;
	float v0 = 0., v1 = 0.;
	// could fail if we have less than 2 time values
	for (int i = 0; i < static_cast<int>(tvs.size()) - 1; i++)
	{
		if (tvs[i]->time <= time && time <= tvs[i + 1]->time)
		{
			i0 = i + 0;
			i1 = i + 1;
			t0 = tvs[i + 0]->time;
	...
	float dvaluedtime0 = 0.;
	float dvaluedtime1 = 0.;
	...
	// we don't want this if have a positively accelerating curve with no speed decay
	if (i1 == static_cast<int>(tvs.size()) - 1) {
		dvaluedtime1 = 0.;
	}
	else {
	...
	// projected change moving from first to second point based purely on
	// derivative at first
	float dvaluedt0 = dvaluedtime0 * dtimedt;
	// projected change moving from first to second point based purely on
	// derivative at second
	...
	float ttt = (time - t0) / (t1 - t0);	// 0. <= ttt <= 1.
